...
- The entrance action is now positioned just beneath the centered card; right metadata was moved away from the index after the first desktop screenshot. A fresh screenshot of those two adjustments is still required.
...
- Source supplied as `15秒厨房Q版角色等面视频 (3)(2).mp4`; its filename describes another project, so the site asset uses `assets/sera-world-welcome.mp4`. The original was 1,650,141 bytes and had an AAC track; the site version is a 704,574-byte, video-only H.264 encode at the original 720×1280 and 24 fps, with `+faststart`. The image comparison of decoded frames measured SSIM 0.983 overall.
